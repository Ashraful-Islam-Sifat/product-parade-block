...
const Spacing = ( {
	...
	const setSpacingData = ( newValue, typeKey ) => {
		...
		if ( ! attributes?.device && ! attributes?.allChange ) {
			setAttributes( {
				[ attributesKey ]: { ...attributes, [ typeKey ]: newValue },
			} );
		}
		...
		if ( attributes?.device && attributes?.allChange ) {
			...
			setAttributes( {
				[ attributesKey ]: {
					...attributes,
					device: { ...attributes.device, [ deviceType ]: data },
				},
			} );
		}
	...
	const activeResetButton = () => {
		if (
			defaultValue?.unit !== attributes?.unit ||
			JSON.stringify( defaultValue.value ) != JSON.stringify( value )
		) {
			return 'active';
		}
		return '';
				...
				<div className="ppb-spacing-part-1">
					<div className="my-range-control-header-area">
						<div className="left-area">
							<label className="my-range-control-label">
								{ label }
							</label>
						...
						<>
							<div className="ppb-spacing-top">
								<input
									id="ppb-spacing-top"
									onChange={ ( e ) =>
										setSpacingData( e.target.value, 'top' )
									}
									type="number"
									value={ value?.top }
								/>
								<label htmlFor="ppb-spacing-top">
									{ labelItem.top }
								</label>
							</div>
							<div className="ppb-spacing-right">
								<input
									...
									value={ value?.right }
								/>
								<label htmlFor="ppb-spacing-right">
									{ labelItem?.right }
								</label>
							</div>
							<div className="ppb-spacing-bottom">
								<input
									...
									value={ value?.bottom }
								/>
								<label htmlFor="ppb-spacing-bottom">
									{ labelItem?.bottom }
								</label>
							</div>
							<div
								className={ `ppb-spacing-left ${
									! linkButton ? 'box' : ''
								}` }
							>
								<input
									id="ppb-spacing-left"
									onChange={ ( e ) =>
										setSpacingData( e.target.value, 'left' )
									}
									type="number"
									value={ value?.left }
								/>
								<label htmlFor="ppb-spacing-left">
									{ labelItem?.left }
								</label>
							</div>
							{ linkButton && (
								<div className={ `ppb-spacing-all` }>
									<Button
										className={
											attributes?.allChange
												? 'active'
												: ''
										}
										onClick={ () =>
